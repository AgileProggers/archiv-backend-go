...
// HasEmotes applies the HasEdge predicate on the "emotes" edge.
func HasEmotes() predicate.Provider {
	return predicate.Provider(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
			sqlgraph.To(EmotesTable, FieldID),
			sqlgraph.Edge(sqlgraph.O2M, false, EmotesTable, EmotesColumn),
		)
		sqlgraph.HasNeighbors(s, step)
	})
}

// HasEmotesWith applies the HasEdge predicate on the "emotes" edge with a given conditions (other predicates).
func HasEmotesWith(preds ...predicate.Emote) predicate.Provider {
	return predicate.Provider(func(s *sql.Selector) {
		step := sqlgraph.NewStep(
			sqlgraph.From(Table, FieldID),
			sqlgraph.To(EmotesInverseTable, FieldID),
			sqlgraph.Edge(sqlgraph.O2M, false, EmotesTable, EmotesColumn),
		)
