...
// QueryClip chains the current query on the "clip" edge.
func (gq *GameQuery) QueryClip() *ClipQuery {
	query := &ClipQuery{config: gq.config}
...
func (gq *GameQuery) QueryVod() *VodQuery {
	query := &VodQuery{config: gq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := gq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (gq *GameQuery) Clone() *GameQuery {
	...
	return &GameQuery{
		...
		withClip:   gq.withClip.Clone(),
		withVod:    gq.withVod.Clone(),
...
// WithClip tells the query-builder to eager-load the nodes that are connected to
// the "clip" edge. The optional arguments are used to configure the query builder of the edge.
func (gq *GameQuery) WithClip(opts ...func(*ClipQuery)) *GameQuery {
	query := &ClipQuery{config: gq.config}
...
func (gq *GameQuery) WithVod(opts ...func(*VodQuery)) *GameQuery {
	query := &VodQuery{config: gq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (gq *GameQuery) sqlAll(ctx context.Context) ([]*Game, error) {
	...
	if gq.withClip != nil || gq.withVod != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := gq.withClip; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Game)
		for i := range nodes {
			if nodes[i].clip_game == nil {
				continue
			}
			fk := *nodes[i].clip_game
			if _, ok := nodeids[fk]; !ok {
				ids = append(ids, fk)
			}
			nodeids[fk] = append(nodeids[fk], nodes[i])
		}
		query.Where(clip.IDIn(ids...))
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "clip_game" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Clip = n
			}
		}
	}

	if query := gq.withVod; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Game)
		for i := range nodes {
			if nodes[i].vod_game == nil {
				continue
			}
			fk := *nodes[i].vod_game
			if _, ok := nodeids[fk]; !ok {
		...
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "vod_game" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Vod = n
			}
