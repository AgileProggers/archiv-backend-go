...
// QueryEmotes chains the current query on the "emotes" edge.
func (pq *ProviderQuery) QueryEmotes() *EmoteQuery {
	query := &EmoteQuery{config: pq.config}
...
func (pq *ProviderQuery) Clone() *ProviderQuery {
	...
	return &ProviderQuery{
		...
		withEmotes: pq.withEmotes.Clone(),
...
// WithEmotes tells the query-builder to eager-load the nodes that are connected to
// the "emotes" edge. The optional arguments are used to configure the query builder of the edge.
func (pq *ProviderQuery) WithEmotes(opts ...func(*EmoteQuery)) *ProviderQuery {
	query := &EmoteQuery{config: pq.config}
...
func (pq *ProviderQuery) sqlAll(ctx context.Context) ([]*Provider, error) {
	...
	if query := pq.withEmotes; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*Provider)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.Emotes = []*Emote{}
		}
		query.withFKs = true
		query.Where(predicate.Emote(func(s *sql.Selector) {
			s.Where(sql.InValues(provider.EmotesColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.provider_emotes
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "provider_emotes" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "provider_emotes" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Emotes = append(node.Edges.Emotes, n)
		}
	}
