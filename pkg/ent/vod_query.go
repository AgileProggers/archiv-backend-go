...
func (vq *VodQuery) QueryClips() *ClipQuery {
	query := &ClipQuery{config: vq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := vq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (vq *VodQuery) QueryGame() *GameQuery {
	query := &GameQuery{config: vq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := vq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (vq *VodQuery) FirstID(ctx context.Context) (id int, err error) {
	var ids []int
...
func (vq *VodQuery) FirstIDX(ctx context.Context) int {
...
func (vq *VodQuery) OnlyID(ctx context.Context) (id int, err error) {
	var ids []int
...
func (vq *VodQuery) OnlyIDX(ctx context.Context) int {
...
func (vq *VodQuery) IDs(ctx context.Context) ([]int, error) {
	var ids []int
...
func (vq *VodQuery) IDsX(ctx context.Context) []int {
	ids, err := vq.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (vq *VodQuery) WithClips(opts ...func(*ClipQuery)) *VodQuery {
	query := &ClipQuery{config: vq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (vq *VodQuery) WithGame(opts ...func(*GameQuery)) *VodQuery {
	query := &GameQuery{config: vq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (vq *VodQuery) sqlAll(ctx context.Context) ([]*Vod, error) {
	...
	if query := vq.withClips; query != nil {
		...
		ids := make(map[int]*Vod, len(nodes))
		for _, node := range nodes {
			ids[node.ID] = node
			fks = append(fks, node.ID)
			node.Edges.Clips = []*Clip{}
		}
		var (
			edgeids []int
			edges   = make(map[int][]*Vod)
		)
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(vod.ClipsPrimaryKey[0], fks...))
			},
		...
		if err := sqlgraph.QueryEdges(ctx, vq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "clips": %w`, err)
		}
		query.Where(clip.IDIn(edgeids...))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		}
		for _, n := range neighbors {
			nodes, ok := edges[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected "clips" node returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Clips = append(nodes[i].Edges.Clips, n)
			}
		}
	}

	if query := vq.withGame; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*Vod)
		for i := range nodes {
		...
		query.Where(predicate.Game(func(s *sql.Selector) {
			s.Where(sql.InValues(vod.GameColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.vod_game
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "vod_game" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "vod_game" returned %v for node %v`, *fk, n.ID)
			}
